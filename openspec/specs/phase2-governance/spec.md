# phase2-governance Specification

## Purpose

定义 phase-2 实施治理的机器可读基础设施：capability matrix（owner/child change/feature flag/八级状态/证据 URI）、umbrella 任务的唯一 child owner 映射、固定且被索引的证据目录布局、归档前拒绝伪造完成的 honesty 检查，以及文档中 charter/实施事实/验收状态三态区分。

## Agent Context

- **源码入口：** `openspec/capabilities/matrix.json`、`matrix.schema.json`、`requirements.json`、`scripts/check-capability-matrix.sh`、`scripts/check-evidence-honesty.sh`、`docs/markflow-core-phase2/evidence/`、`docs/markflow-core-phase2/evidence/INDEX.json`。
- **关联规范：** `ci-openspec-validation`（capability 检查纳入 `validate:openspec`）、`archive-sync-gate`（归档前 honesty 检查）、`documentation-consistency`（追踪源与文档一致性）、`feature-flag-rollback`（flag 注册表）。
- **不变量：** 状态词表固定为 acceptance manual 的八个层级；task ID 至多一个 owner；productAccepted 必须要求全部前置层；证据目录布局固定且被 INDEX.json 索引；归档前证据必须非空、当前 revision、PASS 门禁带证据 URI。
- **验证：** `scripts/check-capability-matrix.sh`；`scripts/check-evidence-honesty.sh`；`npm run validate:openspec`。
