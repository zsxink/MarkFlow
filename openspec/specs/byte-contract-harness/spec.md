# byte-contract-harness Specification

## Purpose

为 MarkFlow 的打开/保存路径建立字节级保真契约：以合成 fixture 矩阵、L0 无编辑保真校验、L1 幸存区间保真校验、负向控制与基线表征测试，把"未编辑文档不得发生任何字节变化"与"编辑只应改变声明范围内的字节"固化为可复现、可机器判定的证据。当前 ProseMirror 路径的尾部换行丢失先以显式表征命令复现并留存失败证据，待后续安全修复落地后转为默认常绿回归。
