...
interface PlantumlContextMenuState {
  svg: string;
  defaultName: string;
}

export function showPlantumlContextMenu(x: number, y: number, state: PlantumlContextMenuState) {
  ...
  showContextMenuStatic(menuItems, { x, y }, { className: 'plantuml-context-menu' });
}

export function hidePlantumlContextMenu() {
}
...
async function saveSvg(state: PlantumlContextMenuState) {
  const saved = await savePlantUmlSvgExport(state.svg, state.defaultName);
  if (saved) {
    showToast('SVG 已保存');
  }
}

async function savePng(state: PlantumlContextMenuState) {
  const pngBlob = await svgToPngBlob(state.svg);
  const data = await blobToBase64(pngBlob);
  const saved = await savePlantUmlPngExport(data, state.defaultName);
  if (saved) {
    showToast('PNG 已保存');
  }
}
