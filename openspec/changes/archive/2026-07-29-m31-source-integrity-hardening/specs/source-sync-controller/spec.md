...
## Purpose
定义前端 SourceSyncController 深模块的行为：使用单 in-flight 并发模型 + ChangeSet.compose + 有界队列 + 严格 flush barrier + recovery replay，替换当前并行 fire-and-forget 模型。
...
- **THEN** base revision 为当前 `confirmedRevision`（所有 change 基于同一起始状态）
