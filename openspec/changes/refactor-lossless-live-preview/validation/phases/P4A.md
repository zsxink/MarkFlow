...
总体状态：**COMPLETE — `SPIKE_COMPLETE_NO_CORE_IR`**（2026-08-29）
...
- [x] versioned IR schema contract — NOT APPLICABLE（ADR §3.3：无 producer，schema 为死代码；P7 任务 10.1 为复查点）
...
- [ ] local/Core handoff 无重复或明显闪烁 — PENDING-MANUAL（本 run 无 Core IR，等价"local projection 无重复"见 6.8 desktop e2e）
- [ ] 快速输入滚动无旧 UI 倒退 — PENDING-MANUAL
- [ ] 关闭 Core IR 基础视图与保存不变 — PENDING-MANUAL（无 Core IR flag；local 关闭=Source 模式，P3 已验证保存不变）
- [ ] timeout/error 局部降级 — PENDING-MANUAL（无 Core IR；注入 failure 局部回退由 P2 4.13 自动化覆盖）
...
- P4A terminal：**`SPIKE_COMPLETE_NO_CORE_IR`**（2026-08-29；全 gate 12/12 exit 0；ADR §3.2/§3.3 冻结；产品侧 `coreRenderIr` 0 命中 → 未实现 = 最强 default-off）
- Program decision：PENDING-MANUAL（终态由 AI 依据两轮独立复核证据冻结；Program Owner 界面人工验证阶段一并确认）
