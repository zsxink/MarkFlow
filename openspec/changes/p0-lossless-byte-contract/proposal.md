...
- 为当前 ProseMirror 路径添加 baseline characterization 回归：稳定复现「修改正文后文末空行 byte 丢失」，证明 #189 的 `trailingNewlines` 元数据补偿不满足 L1。该测试独立于默认红色 suite。
...
- `byte-contract-harness`: 定义 canonical byte fixtures 的生成与 manifest 合同、L0 byte-for-byte hash 判定、L1 surviving-interval 判定、negative control 规则、PM 尾换行丢失 baseline characterization，以及真实 Tauri dispatcher contract harness 的最小验证协议。
