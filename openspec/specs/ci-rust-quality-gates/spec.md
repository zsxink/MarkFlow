# ci-rust-quality-gates Specification

## Purpose
定义 Rust workspace formatting、workspace clippy 和 Core clippy 在 CI 与本地 PR 前验证中的要求，防止局部 Rust 检查替代 CI workspace gate。

## Agent Context
- **源码入口：** `.github/workflows/ci.yml`、`src-tauri/Cargo.toml`、`markflow-core/Cargo.toml`。
- **关联规范：** `ci-core-test`、`issue-workflow`、`archive-sync-gate`。
- **不变量：** Rust-affecting 变更的本地验证不得用单 crate 或单 manifest 命令替代 CI workspace gate。
- **验证：** `cd src-tauri && cargo fmt --all -- --check`；`cd src-tauri && cargo clippy --workspace --all-targets -- -D warnings`；`cd markflow-core && cargo clippy --all-targets -- -D warnings`。
