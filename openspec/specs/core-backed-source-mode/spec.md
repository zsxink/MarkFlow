...
Source Mode 保存 SHALL 只使用 Core `SavePayload` 的输出，不得调用 `getMarkdown()`、`getSourceContent()` 或任何前端 serializer。Save As SHALL 通过创建新 Core session 并执行 `save_document` 完成。
