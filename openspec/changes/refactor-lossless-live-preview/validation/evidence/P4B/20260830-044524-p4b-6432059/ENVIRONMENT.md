# P4B 7.2a final-corrective immutable environment snapshot

| Field | Measured value |
| --- | --- |
| Recorded at | 2026-08-30T04:45:24+0800 |
...
| Predecessors | `20260830-043257-p4b-6432059` (rustfmt FAIL); `20260830-043642-p4b-6432059` (lossless E2E fixture setup FAIL) |
| Candidate corrections | rustfmt-only Core formatting + deterministic creation of five isolated P4B reveal E2E fixtures |
...
| OpenSpec | 1.6.0 |
| P4B flags | `headingStrong` and `quoteLists` default OFF; tests enable/restore explicitly; no table widget registration |
| Real IME / a11y | Not exercised in 7.2a; owned by 7.3/later P4B acceptance |

## Dirty status at run start

The candidate contains the 7.2a product/tests, two preserved failed evidence runs/issues, and the E2E fixture harness correction. The user-supplied untracked GOAL file is excluded. Exact `git status --short`:
...
- Desktop runner creates a repository-local temporary run root and deletes it after success; failures preserve redacted artifacts only.
- Five P4B files are generated inside that isolated workspace from audited constants.
