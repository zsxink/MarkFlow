# wysiwyg-markdown-round-trip Specification
...
当 WYSIWYG 内通过表格交互（边界插入句柄 / 单元格右键菜单）对 GFM 表格执行行列增删、表头切换或对齐设置后，文档切换回源码模式或保存时，序列化输出 MUST 仍是合法 GFM 表格，且重新解析后单元格数据、行内格式与列对齐语义保持不变。交互产生的对齐属性 MUST 通过对齐标记行（`:---` / `:---:` / `---:`）持久化，不得在往返中丢失。
