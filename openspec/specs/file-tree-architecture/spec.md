...
- **不变量：** 规范化 path 是节点和目录状态的稳定键；目录加载只处理直接子项并保留排序；单项事件不得重建整棵树，重命名必须迁移已加载后代及活动/展开状态；增量更新 SHALL 使用 rAF 批量提交，pendingMutations 队列积累 16ms 内变更。
...
文件树模块 SHALL 拆分为四个文件：`fileTree.ts`（公共入口，纯 re-export）、`fileTree.core.ts`（核心渲染、状态管理、排序、增量更新）、`fileTree.dragdrop.ts`（拖拽）、`fileTree.inline.ts`（内联编辑）。`fileTree.ts` SHALL 为纯 re-export 桶文件，不包含任何函数定义。
