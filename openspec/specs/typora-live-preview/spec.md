# typora-live-preview Specification

## Purpose
定义 Typora 级所见即所得（WYSIWYG）实时预览体验：单一 CodeMirror 编辑表面、隐藏非活动 Markdown 标记、显式 projection 生命周期，以及块级稳定排版。使 WYSIWYG 模式在视觉质量、状态可观测性与编辑保真度上达到生产级。
