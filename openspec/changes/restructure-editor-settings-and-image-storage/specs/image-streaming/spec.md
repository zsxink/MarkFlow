...
**Migration**: `storageMode` 不再接受 `'none'` 值。旧设置中 `imageStorageMode` 为 `none` 的迁移到 `workspace-assets`，且 `imageLocalFileBehavior` 设为 `'reference'`（原 `none` 行为的等价替代）。
...
#### Scenario: 本地图片复制到存储
- **WHEN** 用户粘贴/拖入本地图像文件
- **AND** `imageLocalFileBehavior` 设置为 `'copy'`
...
#### Scenario: 本地图片引用原始路径
- **WHEN** 用户粘贴/拖入本地图像文件
- **AND** `imageLocalFileBehavior` 设置为 `'reference'`
- **THEN** 图像路径不被复制到存储位置
...
- **THEN** 图片始终保存到存储位置
- **AND** `imageLocalFileBehavior` 设置不影响剪贴板图片
...
#### Scenario: 网络下载写入临时文件
- **WHEN** `imageNetworkBehavior` 为 `'download'` 且触发网络图片下载
...
- **AND** 与授权根目录（工作区的 `assets/` 或用户自定义路径）比较
